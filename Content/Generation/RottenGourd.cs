...
namespace Spooky.Content.Generation
{
	internal class RottenGourd
	{
		...
		public static void Place(Point16 point)
		{
			...
			int platformType = id switch
			{
				0 => ModContent.TileType<GourdBlockGreenPlatform>(),
				1 => ModContent.TileType<GourdBlockLimePlatform>(),
				2 => ModContent.TileType<GourdBlockOrangePlatform>(),
				3 => ModContent.TileType<GourdBlockRedPlatform>(),
				4 => ModContent.TileType<GourdBlockWhitePlatform>(),
				_ => ModContent.TileType<GourdBlockYellowPlatform>()
			};
			...
			//place platforms inside the gourd
			for (int i = gourdPos.X - 20; i <= gourdPos.X + 20; i++)
			{
				for (int j = gourdPos.Y - 10; j <= gourdPos.Y + 30; j++)
				{
					if (WorldGen.InWorld(i, j, 50))
					{
						...
						int Length = WorldGen.genRand.Next(3, 8);

						if (WorldGen.genRand.NextBool())
						{
							if (tile.WallType == wallType && tile.TileType == tileType && CanPlacePlatform(i, j, Length, false))
							{
								for (int x = i + 1; x <= i + Length; x++)
								{
									WorldGen.PlaceTile(x, j, platformType);
								}
							}
						}
						else
						{
							if (tile.WallType == wallType && tile.TileType == tileType && CanPlacePlatform(i, j, Length, true))
							{
			...
			for (int i = gourdPos.X - 20; i <= gourdPos.X + 20; i++)
			{
				...
				for (int j = gourdPos.Y - 30; j <= gourdPos.Y + 5; j++)
				{
					if (WorldGen.InWorld(i, j, 50))
					{
						...
						if (tile.WallType == wallType && !tile.HasTile && tileAbove.TileType == TileID.LivingWood)
						{
		...
		public static bool CanPlacePlatform(int PositionX, int PositionY, int Length, bool LeftSide)
		{
			if (LeftSide)
			{
				for (int x = PositionX - Length; x <= PositionX - 1; x++)
				{
					for (int y = PositionY - 2; y <= PositionY; y++)
					{
