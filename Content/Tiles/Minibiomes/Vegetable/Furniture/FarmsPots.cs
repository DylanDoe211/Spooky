...
namespace Spooky.Content.Tiles.Minibiomes.Vegetable.Furniture
{
	public class FarmsPots : ModTile
	{
		...
		public override IEnumerable<Item> GetItemDrops(int i, int j)
		{
			switch (Main.rand.Next(5))
			{
				...
				case 0:
				{
					yield return new Item(ItemID.JungleTorch, Main.rand.Next(4, 13));
					break;
				}
				...
				case 2:
				{
					int[] Potions = new int[] { ItemID.IronskinPotion, ItemID.ShinePotion, ItemID.SwiftnessPotion, ItemID.RecallPotion };
					yield return new Item(Main.rand.Next(Potions));
					break;
				}
				//bombs
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			...
			if (Main.netMode != NetmodeID.Server)
			{
				...
				for (int numGores = 1; numGores <= 3; numGores++)
				{
					...
					if (variant == 0)
					{
						Gore.NewGore(WorldGen.GetItemSource_FromTileBreak(x, y), gorePos, goreVelocity, ModContent.Find<ModGore>("Spooky/FarmsPotGore1").Type);
					}
					else if (variant == 1)
					{
						Gore.NewGore(WorldGen.GetItemSource_FromTileBreak(x, y), gorePos, goreVelocity, ModContent.Find<ModGore>("Spooky/FarmsPotGore2").Type);
					}
					else if (variant == 2)
					{
						Gore.NewGore(WorldGen.GetItemSource_FromTileBreak(x, y), gorePos, goreVelocity, ModContent.Find<ModGore>("Spooky/FarmsPotGore3").Type);
					}
				}
			}
		}
