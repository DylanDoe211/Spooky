...
namespace Spooky.Content.NPCs.Quest
{
    public class TarPig : ModNPC  
    {
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) 
        {
            ...
            {
				new FlavorTextBestiaryInfoElement("Mods.Spooky.Bestiary.TarPig"),
		...
		public override void ModifyHoverBoundingBox(ref Rectangle boundingBox)
		{
			...
			if (NPC.ai[0] == 0)
			{
				boundingBox = new Rectangle((int)NPC.position.X, (int)NPC.position.Y + NPC.height - HeightOffset - NPC.height / 2, NPC.width, NPC.height / 2);
			}
			else
			{
			...
			switch ((int)NPC.ai[0])
			{
				...
				case 0:
				{
					if (NPC.wet && WorldGen.InWorld((int)(NPC.Center.X + (float)((NPC.width / 2 + 8) * NPC.direction)) / 16, (int)(NPC.Center.Y / 16f), 5))
					{
						...
						NPC.velocity.X = (NPC.velocity.X * 18f + 1f * (float)NPC.direction) / 20f;
					...
					else
					{
						...
						if (NPC.Distance(player.Center) <= 300f && HasLineOfSight)
						{
							NPC.ai[0]++;
							NPC.netUpdate = true;
						}
				...
				case 1:
				{
					...
					if (NPC.localAI[0] == 2)
					{
						SoundEngine.PlaySound(SoundID.NPCHit56 with { Volume = 0.5f, Pitch = 0.5f }, NPC.Center);
					}

					if (NPC.localAI[0] >= 40 && NPC.localAI[0] % 7 == 0)
					{
						SoundEngine.PlaySound(SoundID.Item171 with { Pitch = -1f }, NPC.Center);
						...
						NPCGlobalHelper.ShootHostileProjectile(NPC, NPC.Center + new Vector2(16 * NPC.direction, -5), newVelocity, ModContent.ProjectileType<HydroraptorSpit>(), NPC.damage, 4.5f);
					}
		...
		public override void HitEffect(NPC.HitInfo hit) 
        {
            if (NPC.life <= 0) 
            {
                ...
                for (int numGores = 1; numGores <= 5; numGores++)
                {
                    ...
                    {
                        Gore.NewGore(NPC.GetSource_Death(), OffsetCenter, NPC.velocity, ModContent.Find<ModGore>("Spooky/TarPigGore" + numGores).Type);
                    }
                }
            }
        }
